...
describe('Platzi Punks Contract', () => {
  ...
  describe('Deployment', () => {
    it('Sets max suppply to passed param', async () => {
  ...
  describe('Token URI', () => {
    it('Returns valid metadata', async () => {
      ...
      const stringtifiedTokenURI = await tokenURI.toString();
      const [, base64JSON] = stringtifiedTokenURI.split(
        'data:application/json;base64'
      );
      const stringtifiedMetadata = await Buffer.from(
        base64JSON,
        'base64'
      ).toString('ascii');

      const metadata = JSON.parse(stringtifiedMetadata);
